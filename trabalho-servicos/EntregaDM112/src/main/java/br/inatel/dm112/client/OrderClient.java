...
public class OrderClient {
	...
	public void startOrderDelivery(int orderNumber) {
		String url = restURL + endpoint + "/" + orderNumber + "/shipped";
		System.out.println("URL: " + url);
		makePUTCallWithEmptyBody(orderNumber, url);
		System.out.println("Sucesso ao iniciar a Entrega para o pedido: " + orderNumber);
	}

	public void confirmOrderDelivery(int orderNumber) {
		String url = restURL + endpoint + "/" + orderNumber + "/delivered";
		makePUTCallWithEmptyBody(orderNumber, url);
		System.out.println("Sucesso ao confirmar a Entrega para o pedido: " + orderNumber);
	}
	...
	private void makePUTCallWithEmptyBody(int orderNumber, String url) {
		try {
			...
			if(response.statusCode() != 204) {
				throw new RuntimeException("Error updating Order Delivery. statusCode: " + response.statusCode() + " expected: 204");
			}
		} catch (Exception e) {
			e.printStackTrace();
			throw new RuntimeException("Error updating Order Delivery: " + e.getMessage());
		}
	}
